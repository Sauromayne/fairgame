...
* By default, FairGame Alpha will only purchase new items with free shipping. Please see [Configuration](#Configuration) 
  if you would like to change this.
* FairGame Alpha is designed to check if each product is in stock simulaneously. It does this by creating a monitoring session for each ASIN in your `amazon_aio.config` file. If   you're checking 5 ASIN's with your configuration, all 5 ASIN's will be checked every 5 seconds by default with their own monitoring session that is not logged in to your         Amazon account. If you're using proxies each monitoring session will be running through one proxy in your `proxy.json` file. If you're not using proxies each monitoring         session will run on your home IP. *DO NOT* expect support if you are running several ASIN's without proxies and get softbanned. See [Disclaimer](#Disclaimer).
